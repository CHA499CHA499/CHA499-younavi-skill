...
扫描、规范化、精确去重、落盘和分批均不调用模型。只有第 4 步开始消耗 token，每批正文最多约
60,000 tokens，包含指令的完整输入不超过约 64,000 tokens。首次同意覆盖本次冻结出的全部历史批次，
批次会在后台逐个串行完成，不再每 4 批要求用户继续；状态会显示
...
历史扫描和提炼由异步 `task.completed` hook 驱动，不占住用户当前对话。流程进入 `completed` 或
`failed` 后，插件会通过 `source=cinder_memory_history_notice` 在 YouNavi 创建一次
可见回执；完成回执包含扫描数、唯一材料数、精确重复数、批次数、新增长期记忆数和待确认数。该通知
需要一个很小的 `simple_chat` 模型回复，因为当前外置插件没有零 token 的原生 Toast 接口；通知会被 hook
与历史扫描排除，不会形成递归或污染记忆。
...
当前完成事件自身处理失败时，hook 仍会独立执行一次过期任务对账，避免同一前置错误把历史批次永久挡住。
...
历史回填使用同一 hook 的 `source=cinder_memory_history_extract` 分支：每批正文最多约 60,000 tokens，
完成后本地应用计划，再启动
...
终态回执先在状态文件中原子认领，再创建 `simple_chat` conversation；同一终态只发一次。若创建结果
不确定，插件失败关闭且不重发，避免重复通知。回执 conversation 以专用 source 标记并在所有采集路径
最前面跳过。

所有 `agent-cli` 子进程 stdout/stderr 分别设 4,000,000/64,000 字节硬上限；超过即终止该子进程并失败
关闭。hook 对 YouNavi 只输出一行固定小摘要，外部错误会先脱敏、折叠为单行、截到 480 字符并附
`error_id`。同一链路连续失败 3 次后按 5 分钟、30 分钟、2 小时、6 小时退避，成功后立即解除；晚报和
已创建提取任务的完成事件不被普通退避跳过。`.state/capture-health.json` 只保留聚合计数和最近摘要，
不会按失败次数追加无界日志。当前 YouNavi 的完成事件可能不带 `task_source`；退避期会在用户身份校验
后读取一次 conversation source，再决定跳过普通任务或继续关键任务，该响应会被后续处理复用。
